...
class RapidDNSSource(DomainSource):
    ...
    def _get_total_results(self, soup):
        ...
        return int(span.text.strip()) if span and span.text.strip().isdigit() else 0
...
class YouGetSignalSource(DomainSource):
    ...
    def fetch(self, ip):
        ...
        if response:
            domains.update(domain[0] for domain in response.json().get("domainArray", []))
        return domains

def get_scrapers():
    return [RapidDNSSource(), YouGetSignalSource()]